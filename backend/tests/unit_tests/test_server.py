...
def test_recommend_stream_error(client, mocker):
    ...
    mock_stream = mocker.MagicMock()
    mock_stream.return_value = iter([
        'data: {"error": "LLM Streaming error"}\n\n'
    ])
    mocker.patch("backend.server.stream_recommend", mock_stream)
    ...
    assert response.status_code == 200 # SSE responses should not use 500, instead return errors in the stream
...
def test_chat_non_existent_session(client, mocker):
    ...
    assert "data: {\"content\": \"This is the first response in a new session.\"}" in response_data
